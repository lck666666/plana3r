...
#define MAX_PLANE_NUM 5000
#define DEBUG_PIXID 90535
#define DEBUG_CUDA_ON 0
...
__device__ const float cutoff = 1.2f;
...
{
	rect_min = {
		min(grid.x, max((int)0, (int)((p.x - max_radius * 1.0f) / BLOCK_X))),
		min(grid.y, max((int)0, (int)((p.y - max_radius * 1.0f) / BLOCK_Y)))
	};
	rect_max = {
		min(grid.x, max((int)0, (int)((p.x + max_radius * 1.0f + BLOCK_X - 1) / BLOCK_X))),
		min(grid.y, max((int)0, (int)((p.y + max_radius * 1.0f + BLOCK_Y - 1) / BLOCK_Y)))
	};
