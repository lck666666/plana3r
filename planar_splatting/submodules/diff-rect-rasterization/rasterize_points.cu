	...
	const bool hard_render)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		debug,
		lambda,
		image_center.contiguous().data<float>(),
		hard_render);
  }
...
//   torch::Tensor dL_dscales = torch::zeros({P, 2}, means3D.options());
  torch::Tensor dL_dscales = torch::zeros({P, 4}, means3D.options());
  torch::Tensor dL_drotations = torch::zeros({P, 4}, means3D.options());

  torch::Tensor dL_dproj = torch::zeros({P, 2}, means3D.options());
  torch::Tensor dL_dtransMat2 = torch::zeros({P, 9}, means3D.options());
  ...
  {  
	  ...
	  dL_dtransMat2.contiguous().data<float>(),
	  image_center.contiguous().data<float>(),
