...
import plana3r.utils.path_to_croco  # noqa: F401
from plana3r.model_plana3r_v2 import AsymmetricCroCo3DStereo, Plana3rModel, inf  # noqa: F401, needed when loading the model
from plana3r.model_plana3r_v1 import Plana3rModelNaive  # noqa: F401
from plana3r.datasets import get_data_loader  # noqa
from plana3r.losses_dust3r import *  # noqa: F401, needed when loading the model
from plana3r.loss_plana3r import loss_of_one_batch_plane # noqa
from plana3r.utils import croco_misc as misc  # noqa


def get_args_parser():
    parser = argparse.ArgumentParser('PLANA3R training', add_help=False)
    # model and criterion
    ...
    parser.add_argument('--no_rast_epochs', default=1, type=int, help="epoches disable rasterization")
    return parser
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        ...
        if (epoch > 0 and args.eval_freq > 0 and epoch % args.eval_freq == 0):
            ...
            for test_name, testset in data_loader_test.items():
                stats = test_one_epoch(model, testset, 
                                       device, epoch, log_writer=log_writer, args=args, prefix=test_name, criterion=test_criterion)
...
def train_one_epoch(model: torch.nn.Module, data_loader: Sized, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, args, log_writer=None, criterion=None):
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        loss_plane, loss_details = loss_of_one_batch_plane(
            batch, model, device, symmetrize_batch=True, use_amp=bool(args.amp), ret='loss', 
            epoch=epoch, itr=data_iter_step, no_rast_epochs=args.no_rast_epochs, criterion=criterion)
        
        loss_value = float(loss_plane)
        loss = loss_plane
...
def test_one_epoch(model: torch.nn.Module,
                   ...
                   args, log_writer=None, prefix='test', criterion=None):
    ...
    for _, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        # loss_value, loss_details_3dpts = loss_tuple 
        
        loss_plane, loss_details = loss_of_one_batch_plane(batch, model, device,
                                       symmetrize_batch=True,
                                       use_amp=bool(args.amp), ret='loss',epoch=epoch,itr=-1, criterion=criterion)
